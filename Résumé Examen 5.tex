...
With those development factors, we can calculate the ultimate ALAE. For example, for accident year 2013 using the additive factors: 
\begin{align*}
\text{Ultimate Ratio} &= 0.138 + 0.135 = 0.272\\[7pt]
\text{Ultimate ALAE} &= \text{Ultimate Claims}\times\text{Ultimate Ratio}\\
&= 0.272 * 5493\$\\
&= 1497\$
\end{align*}
...
\subsection{Simplified Generalized Approach}	
	...
	&= W^{*} \times \left[ U_{1}(\text{Pure IBNR}) + U_{2}(\text{Case Oustanding} + \text{IBNER}+ \text{Pure IBNR})\right]\\
