...
\textbf{For this section, we have to know the following notation : }
	\begin{description} 
		\item[M:] Paid ULAE
		\item[W:] Ratio of ULAE to claims
		\item[B:] Claim basis for the calendar year
		\item[R:] Ultimate cost of claims reported (ie. opened) during the calendar year
		\item[P:] Paid claims during the calendar year
		\item[C:] Ultimate cost of claims closed during the calendar year.
...
\begin{align*}
\mathbf{M= WB = W\left(U_{1}R + U_{2}P + U_{3}C\right)}
\end{align*}
\end{tcolorbox}
	...
	In this technique, the unpaid ULAE are estimated using the ratio of \emph{paid ULAE} to \emph{paid claims}. The assumptions of this method is that the ratio is generally stable through time and that the cost of future claims management activity will be proportional to the IBNR and case outstanding. 
	
	The technique assumes that half of the ULAE are sustained when a claim is opened, while the other half is sustained when it is closed. Hence, \emph{we apply 50\% of the ratio to case outstanding} (since the first half of the expenses is already incurred) and \emph{100\% of the ratio to the IBNR} (since it's not yet reported, no expenses has been incurred). Here are the steps to apply the method:
	\begin{enumerate}
		\item \textbf{Calculated the ratios of historical \emph{paid ULAE} to \emph{paid claims} by calendar year.}\\
	\end{enumerate}
